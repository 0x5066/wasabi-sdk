...
int FrameWnd::onInit() {
  ...
  if (getName() != NULL) {
    StringPrintf buf("FrameWnd/ws,%s", getName());
    int ws = api->getIntPrivate(buf, /*rwchildren[0] && rwchildren[0]->childNotify(NULL, CHILD_WINDOWSHADE_CAPABLE)*/ 0);
    if (ws) {
      windowshade(0, !ws);
      windowshade(0, ws);
      pullbarpos = 0;
...
int FrameWnd::getCursorType(int x, int y) {
  ...
  getClientRect(&r);
  POINT pt={x,y};
  if (y > r.top + getLabelHeight() && PtInRect(&sizerRect, pt)) {
  ...
  if (slidemode == FRAMEWND_COVER) { // cover mode
    ...
    if (vert == DIVIDER_VERTICAL) {
      ...
      if (divideside == SDP_FROMLEFT) { // from left
        ...
        SetRect(&r1, r.left, r.top, w, r.bottom-r.top);
        SetRect(&r2, r.left+w+sizerwidth, r.top, (r.right-r.left)-(w+sizerwidth), r.bottom-r.top);
        ...
        //FG> Warning, this is using a rect for x,y,W,H and NOT l,r,t,b
        SetRect(&r1, r.left, r.top, w-sizerwidth, r.bottom-r.top);
        SetRect(&r2, r.left+w, r.top, (r.right-r.left)-w, r.bottom-r.top);
      ...
      //FG> Warning, this is using a rect for x,y,W,H and NOT l,r,t,b
      SetRect(&r1, r.left, r.top, r.right-r.left, w); 
