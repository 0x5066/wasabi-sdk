...
void ButtonWnd::reloadResources() {
  ...
  if (normalbmp.getBitmap())
    normalrgn = new RegionI(normalbmp.getBitmap(), 0, 0);
  if (pushedbmp.getBitmap())
    pushedrgn = new RegionI(pushedbmp.getBitmap(), 0, 0);
  if (hilitebmp.getBitmap())
    hirgn = new RegionI(hilitebmp.getBitmap(), 0, 0);
  if (activatedbmp.getBitmap())
    activatedrgn = new RegionI(activatedbmp.getBitmap(), 0, 0);
  currgn = normalrgn;
}
