...
ScrlBkgWnd::~ScrlBkgWnd() {
  ...
  delete hSep; hSep = NULL;
  delete vSep; vSep = NULL;
}
...
void ScrlBkgWnd::_setSlidersPosition() {
  ...
  needSetSliders=FALSE;
	...
	if (needHScroll()) {
		...
		if (!EqualRect(&r, &z)) { // assumes ScrollBars are virtual
...
void ScrlBkgWnd::makeWindowOverlayMask(Region *r) {
  ...
  if (getRandomRgn) {
    ...
    getRandomRgn(dc, sr->getOSHandle(), SYSRGN);
