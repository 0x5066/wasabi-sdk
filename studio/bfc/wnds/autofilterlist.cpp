...
void AutoFilterList::mqc_onNewMultiQuery(SubQueryServer *modifier, int flag) {
  ...
  if (need_reset) {
    ...
    scannerserver_getScanner()->getScanner()->cancelQuery();
    local_scanner->getScanner()->cancelQuery();
...
int AutoFilterList::onDeferredCallback(int p1, int p2) {
  if (p1 == 7873 && p2 == 3245) {
    deselectAll();
    setItemFocused(0);
    setSelected(0, TRUE);
...
int AutoFilterList::onBeginDrag(int iItem) {
  ...
  String val = (fli->getDatatype() == MDT_STRINGZ) ? fli->getData() : StringPrintf(*(int *)fli->getData());
