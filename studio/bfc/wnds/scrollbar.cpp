...
void ScrollBar::loadBmps() {
  ...
  if (bmpleft) leftrgn = new RegionI(bmpleft);
  if (bmpbutton) buttonrgn = new RegionI(bmpbutton);
  if (bmpright) rightrgn = new RegionI(bmpright);
...
int ScrollBar::pageUp() { 
  ...
  setPrivatePosition((int)max(0, (float)getPosition() + (float)SCROLLBAR_FULL / (npages-1)), TRUE, 1/*Std::osparam_getSmoothScroll()*/);
...
int ScrollBar::pageDown() { 
  ...
  setPrivatePosition((int)min(SCROLLBAR_FULL, (float)getPosition() - (float)SCROLLBAR_FULL / (npages-1)), TRUE, 1/*Std::osparam_getSmoothScroll()*/);
