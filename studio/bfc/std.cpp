...
void *DO_MALLOC(int size EXTRA_INFO) {
 ...
 #else
  void *p = malloc(size);
  if (p != NULL) MEMSET(p, 0, size);//BU added in response to talkback info
  return p;
 #endif
#else
 #ifndef USE_MALLOC
...
void *DO_REALLOC(void *ptr, int size EXTRA_INFO) {
 ...
 #else
  int oldsize = _msize(ptr);
  r = _realloc_dbg(ptr, size, _NORMAL_BLOCK, file, line);
 #endif
#endif
  ...
  if (!r) { // realloc failed
    ...
    int s = _msize(ptr);
#endif
    MEMCPY(newblock, ptr, MIN(s, size));
    FREE(ptr);
    r = newblock;
  } 
...
char *DO_STRDUP(const char *ptr EXTRA_INFO) {
  ...
  ret = (char *)DO_MALLOC(STRLEN(ptr)+1 EXTRA_PARAMS);
  if (ret != NULL) {
    STRCPY(ret, ptr);
  }
  return ret;
...
wchar_t *DO_WSTRDUP(const char *ptr8 EXTRA_INFO) {
  ...
  ret = (wchar_t *)DO_MALLOC((size8 + 1) * sizeof(wchar_t) EXTRA_PARAMS);
  if (ret != NULL) {
    int i;
    for (i = 0; i < size8; i++) {
...
unsigned long BSWAP(unsigned long input) {
#ifdef _MSC_VER
  _asm {
    mov eax, input
    bswap eax
...
void Std::getViewport(RECT *r, POINT *p, int full) {
  getViewport(r, p, NULL, NULL, full);
}

void Std::getViewport(RECT *r, RECT *sr, int full) {
  getViewport(r, NULL, sr, NULL, full);
}

void Std::getViewport(RECT *r, HWND wnd, int full) {
  getViewport(r, NULL, NULL, wnd, full);
}

void Std::getViewport(RECT *r, POINT *p, RECT *sr, HWND wnd, int full) {
  ASSERT(r != NULL);
	if (p || sr || wnd) {
...
BOOL CALLBACK enumViewportsProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
...
int Std::messageBox(const char *txt, const char *title, int flags) {
...
int Std::isRootPath(const char *path) {
  ...
  if (path[0] == DIRCHAR) return 1;
#ifdef WIN32
  // extra win32 check for drive letter i.e. c:\ 
  if (ISALPHA(path[0]) && path[1] == ':' && path[2] == DIRCHAR) return 1;
#endif
  return 0;
}
...
int FSEEK(FILE *stream, long offset, int origin) {
  if (fileReaders.searchItem((void *)stream)!=-1)
    return api->fileSeek(offset, origin,(void *)stream);
  return fseek(stream,offset,origin);
}

long FTELL(FILE *stream) {
  if (fileReaders.searchItem((void *)stream)!=-1)
    return api->fileTell((void *)stream);
  return ftell(stream);
}

size_t FREAD(void *buffer, size_t size, size_t count, FILE *stream) {
  if (fileReaders.searchItem((void *)stream)!=-1)
    return api->fileRead(buffer,size*count,(void *)stream);
  return fread(buffer,size,count,stream);
}

size_t FWRITE(const void *buffer, size_t size, size_t count, FILE *stream) {
  if (fileReaders.searchItem((void *)stream) != -1)
    return api->fileWrite(buffer,size*count,(void *)stream);
  return fwrite(buffer,size,count,stream);
}

int FGETSIZE(FILE *stream) {
  if (fileReaders.searchItem((void *)stream)!=-1)
    return api->fileGetFileSize((void *)stream);
  int p = ftell(stream);
...
const char *TMPNAM2(char *str, int val) {
#ifdef WIN32
  String tempPath8;
  ...
  if (Std::encodingSupportedByOS(SvcStrCnv::UTF16)) {
    ...
    GetTempFileNameW(tempPath16, L"DTC", val, tempName16);
    // Convert
    String tempN8;
    ...
    GetTempFileName(tempPathOS, "DTC", val, tempNameOS);
    // Convert
    String tempN8;
...
int MOVEFILE(const char *filename, const char *destfilename) {
  return api->fileMove(filename, destfilename);
}
...
void Std::setThreadPriority(int delta) {
#ifdef WIN32
  ...
  SetThreadPriority(GetCurrentThread(), v);
#else
#error port me!
#endif
}
...
#if 0
  //return (access(filename, 0) == 0); // gives bad linker warnings and forces io.h in
...
void Std::shellExec(const char *cmd) {
#ifdef WIN32
  ShellExecute(NULL,"open",cmd,NULL,".",0);
#else
#error port me
#endif
}
