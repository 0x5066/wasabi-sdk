...
// this window simply instantiates an xml group called 'ffc.main.window'
...
void MainWnd::hookControls() {
  ...
  if (!_group) {
    api->messageBox("Group \"ffc.main.window\" not found, can't continue", "FreeForm Component Example", MSGBOX_OK, NULL, NULL);
    return;
  }

  // the skinned group is now created, lets wrap a helper around it to get our layer
  group = new C_Group(_group);
  ScriptObject *layer = group->getObject("ffc.layer1");

  if (!layer) {
    api->messageBox("Could not find layer \"ffc.layer1\" in group \"ffc.main.window\", can't continue", "FreeForm Component Example", MSGBOX_OK, NULL, NULL);
    delete group; group = NULL;
    return;
  }
...
const char *MainWnd::getWindowTypeName() { return /**/"ExampleFreeFormWac Window"/*EDITME*/; }
